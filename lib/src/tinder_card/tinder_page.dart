...
class _TinderPageViewState extends State<TinderPageView>
  ...
  Future<void> _fetchProjects() async {
    try {
      final response = await supabase
          .from('projects')
          .select(
              'id, description, tech_stack, development_tags, company, website, industry, hiring_manager_id')
        ...
        setState(() {
          stack = response.map((item) {
            return {
              ...
              "hiring_manager_id": item["hiring_manager_id"] ?? "No hiring manager",
            };
          }).toList();
          isLoading = false;
        });
  ...
  void _swipeCard(bool isRightSwipe) {
    ...
    final project = stack.last;
    final hmId = project['hiring_manager_id'];
    final studentId = widget.userId;

    if (hmId == null) {
      print("Error: hiring_manager_id is missing for the project.");
      setState(() {
        stack.removeLast(); // Remove the problematic project from the stack
      });
      return;
    }
  ...
  Future<void> _registerSwipe(
      String studentId, String hmId, bool isRightSwipe) async {
